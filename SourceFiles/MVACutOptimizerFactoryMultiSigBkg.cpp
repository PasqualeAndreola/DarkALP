                                      ...
                                      bool debug_variableranking)
{
    ...
    Int_t sig_nentries = 0, bkg_nentries = 0;
    ...
    for (unordered_map<string, pair<string, string>>::iterator mvasigfileit = mva_cut_sig_factory_files.begin(); mvasigfileit != mva_cut_sig_factory_files.end(); mvasigfileit++)
    {
        ...
        sig_nentries = sig_nentries+signal->GetEntries();
    }    
    ...
    for (unordered_map<string, pair<string, string>>::iterator mvasigfileit = mva_cut_sig_factory_files.begin(); mvasigfileit != mva_cut_sig_factory_files.end(); mvasigfileit++)
    {
        ...
        signal_dataframe->Range(0, round(stof(mvasigfileit->first)*sig_nentries)).Snapshot(output_treename, output_filename, vars_tobe_written);
        signal_dataframe->Range(round(stof(mvasigfileit->first)*sig_nentries), 0).Snapshot(output_treename, output_test_filename, vars_tobe_written);
    ...
    for (unordered_map<string, pair<string, string>>::iterator mvabkgfileit = mva_cut_bkg_factory_files.begin(); mvabkgfileit != mva_cut_bkg_factory_files.end(); mvabkgfileit++)
    {
        ...
        snapopt.fMode = "RECREATE";
        snapopt.fOverwriteIfExists = "TRUE";
        background_dataframe->Range(0, round(stof(mvabkgfileit->first)*bkg_nentries)).Snapshot(output_treename, output_filename, vars_tobe_written);
        background_dataframe->Range(round(stof(mvabkgfileit->first)*bkg_nentries), 0).Snapshot(output_treename, output_test_filename, vars_tobe_written);
    ...
    long unsigned int ntrainsign = sig_nentries*train_test_treshold; /*!< Number of events used to train signal identification in the sample */
    long unsigned int ntrainback = bkg_nentries*train_test_treshold/80; /*!< Number of events used to train background identification in the sample */
    long unsigned int ntestsign = sig_nentries*(1-train_test_treshold);  /*!< Number of events used to test signal identification in the sample */
    long unsigned int ntestback = (bkg_nentries/80)*(1-train_test_treshold);  /*!< Number of events used to test background identification in the sample */
